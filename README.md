...
- 게시판
  - 게시물 리스트 보기  
    - /board/list.do
    - 게시물 10개 마다 잘라서 분할하여 나타내는 페이지 기능  
    - 게시물 제목, 내용, 작성자에 대해 **검색 기능**  
    - 각 게시물 제목을 클릭하여 게시물 내용을 볼 수 있음(/board/view.do)  
  - 게시물 쓰기
    - '신규 글 작성' 버튼을 통해 신규 글 등록 가능  
    - 제목, 내용, 작성자, 비밀번호를 작성하고 신규 글 작성 가능  
    - 빈 칸이 있으면 등록되지 않음  
  - 게시물 보기  
    - 게시물 내용과 상세 정보를 볼 수 있음  
    - 게시물 수정, 삭제, 답글달기 기능 사용 가능  
  - 게시물 수정  
    - 비밀번호 입력시 수정 가능  
    - 기존 내용이 입력되어 있고 수정 후 저장 가능  
  - 게시물 삭제  
    - 비밀번호 입력시 삭제 가능
  - 답글 달기
    - 게시물 보기에서 답글달기 버튼을 눌러, 답글 기능 사용 가능  
    - 답글을 달면 게시물 번호와는 상관없이 해당 글 바로 위에 등록이 되어 어떤 글의 답글인지 알 수 있음  
    - 답글은 여러개 달 수 있지만, 답글에 답글을 달아도 원래 글에 답글이 달림  

- 로그인 기능 추가
  - 등록된 아이디와 비밀번호로 로그인 가능  
  - ID에 일반 / 관리자 사용권한이 부여됨  
  - 로그인 사용자는 게시물 작성시 작성자, 비밀번호를 쓰지 않아도 게시물 등록 및 수정 가능  
  - 로그인 사용자가 등록한 게시물은 사용자 ID 또는 관리자만 수정 및 삭제 가능  
  - 관리자는 모든 글에 대해 수정 및 삭제 가능
  - 관리자는 회원 관리 페이지에서 다른 사용자에게 일반/관리자 권한 부여 가능  
